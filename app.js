...
function renderCharts(){
 ...
 document.querySelector('#charts').innerHTML=tasks.map(t=>{
  ...
  const bars=t.indices.map((idx,c)=>keys.map((k,j)=>{const m=methods[k],v=m.values[idx],x=25+c*groupWidth+groupWidth/2+(j-keys.length/2)*bw,y=205-v*4.3;return `<g><title>${t.name}, ${t.conditions[c]}: ${m.label}, ${v} of 40 successes</title><rect x="${x}" y="${y}" width="${bw-3}" height="${Math.max(v*4.3,1)}" fill="${m.color}"/><text x="${x+(bw-3)/2}" y="${y-7}" text-anchor="middle" font-size="${keys.length===4?10:12}">${percent?(v*2.5)+'%':v}</text></g>`}).join('')).join('');
  const grid=[0,10,20,30,40].map(v=>`<line x1="25" x2="325" y1="${205-v*4.3}" y2="${205-v*4.3}" stroke="#cbd2c2" stroke-width=".7"/><text x="19" y="${209-v*4.3}" text-anchor="end" font-size="9" fill="#65705d">${percent?v*2.5:v}</text>`).join('');
  const labels=t.conditions.map((c,i)=>`<text x="${25+groupWidth*(i+.5)}" y="228" text-anchor="middle" font-size="12">${c}</text><text x="${25+groupWidth*(i+.5)}" y="246" text-anchor="middle" font-size="9" fill="#65705d">${t.notes[i]}</text>`).join('');
